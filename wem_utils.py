...
def list_to_ids(s_list, tokenizer):
    # convert list of strings to list of list of token ids
    result = []
    for item in s_list:
        tokens = item.split()
...
def edit_embedding_lookup(embedding_table, list_input_ids, use_one_hot_embeddings, vocab_size, embedding_size):
