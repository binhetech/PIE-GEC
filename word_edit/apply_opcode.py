...
def add_arguments(parser):
    ...
    parser.add_argument("--input_tokens", type=str, required=True, help="path to possibly incorrect token file")
    parser.add_argument("--output_tokens", type=str, required=True, help="path to edited (hopefully corrected) file")
    parser.add_argument("--infer_mode", type=str, default="conll", help="post processing mode bea or conll")
    parser.add_argument("--edit_ids", type=str, required=True, help="path to edit ids to be applied on input_tokens")
    parser.add_argument("--path_common_inserts", type=str, required=True, help="path of common unigram inserts")
    parser.add_argument("--path_common_deletes", type=str, required=True,
                        help="path to common deletions observed in train data")
    parser.add_argument("--path_common_multitoken_inserts", type=str, required=True,
                        help="path of common bigram inserts")
...
def apply_opcodes(words_uncorrected, ops_to_apply, opcodes, basic_tokenizer, INFER_MODE="conll",
    ...
    words_corrected = join_subwords(words_corrected, INFER_MODE)
...
def join_subwords(word_list, INFER_MODE):
    global vocab
    tmp_word_list = []
...
if __name__ == "__main__":
    ...
    if not DO_PARALLEL:
        ...
        result = Parallel(n_jobs=-1)(delayed(split_and_convert_to_ints)(*s) for s in
                                     tqdm(zip(words_uncorrected, edits), total=len(words_uncorrected)))
    ...
    with open_w(config.OUTPUT_CORRECTED_WORDS) as out_file:
        ...
        if DO_PARALLEL:
            s_corrected = Parallel(n_jobs=-1)(
                delayed(apply_opcodes)(*s) for s in
                tqdm(zip(words_uncorrected, edits, opcodes, basic_tokenizer, FLAGS.infer_mode),
                     total=len(words_uncorrected)))
            ...
            for i in tqdm(range(len(edits))):
                ...
                s_corrected = apply_opcodes(words_uncorrected[i], edits[i], opcodes, basic_tokenizer, FLAGS.infer_mode)
